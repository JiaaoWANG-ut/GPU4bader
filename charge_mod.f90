...
MODULE charge_mod
  ...
  TYPE :: charge_obj
    ...
    REAL(q2),ALLOCATABLE,DIMENSION(:,:,:) :: rho_pad
  ...
  PUBLIC :: build_scell, init_rho_pad
...
!-----------------------------------------------------------------------------------!
  ...
  SUBROUTINE assign_charge(chg1,chg2)
    ...
    IF (ALLOCATED(chg2%rho_pad)) THEN
      IF (ALLOCATED(chg1%rho_pad)) DEALLOCATE(chg1%rho_pad)
      ALLOCATE(chg1%rho_pad( &
        LBOUND(chg2%rho_pad,1):UBOUND(chg2%rho_pad,1), &
        LBOUND(chg2%rho_pad,2):UBOUND(chg2%rho_pad,2), &
        LBOUND(chg2%rho_pad,3):UBOUND(chg2%rho_pad,3)))
      chg1%rho_pad = chg2%rho_pad
    END IF
...
!-----------------------------------------------------------------------------------!
  ...
  FUNCTION rho_val(chg,p1,p2,p3)
    ...
    rho_val = chg%rho(MODULO(p1-1, chg%npts(1)) + 1, &
                      MODULO(p2-1, chg%npts(2)) + 1, &
                      MODULO(p3-1, chg%npts(3)) + 1)
...
!-----------------------------------------------------------------------------------!
  ...
  SUBROUTINE pbc(p,pmax)
    ...
    p(1) = MODULO(p(1)-1, pmax(1)) + 1
    p(2) = MODULO(p(2)-1, pmax(2)) + 1
    p(3) = MODULO(p(3)-1, pmax(3)) + 1
...
!-----------------------------------------------------------------------------------!
! init_rho_pad: Allocate and fill ghost-layer-padded copy of rho.
!   rho_pad(0:n1+1, 0:n2+1, 0:n3+1) allows direct indexing for +-1 neighbors
!   without any PBC checks.
!-----------------------------------------------------------------------------------!

  SUBROUTINE init_rho_pad(chg)
    ...
    IF (ALLOCATED(chg%rho_pad)) DEALLOCATE(chg%rho_pad)
    ALLOCATE(chg%rho_pad(0:n1+1, 0:n2+1, 0:n3+1))

    chg%rho_pad(1:n1, 1:n2, 1:n3) = chg%rho(1:n1, 1:n2, 1:n3)

    chg%rho_pad(0,    1:n2, 1:n3) = chg%rho(n1, 1:n2, 1:n3)
    chg%rho_pad(n1+1, 1:n2, 1:n3) = chg%rho(1,  1:n2, 1:n3)
    chg%rho_pad(1:n1, 0,    1:n3) = chg%rho(1:n1, n2, 1:n3)
    chg%rho_pad(1:n1, n2+1, 1:n3) = chg%rho(1:n1, 1,  1:n3)
    chg%rho_pad(1:n1, 1:n2, 0)    = chg%rho(1:n1, 1:n2, n3)
    chg%rho_pad(1:n1, 1:n2, n3+1) = chg%rho(1:n1, 1:n2, 1)

    chg%rho_pad(0,    0,    1:n3) = chg%rho(n1, n2, 1:n3)
    chg%rho_pad(0,    n2+1, 1:n3) = chg%rho(n1, 1,  1:n3)
    chg%rho_pad(n1+1, 0,    1:n3) = chg%rho(1,  n2, 1:n3)
    chg%rho_pad(n1+1, n2+1, 1:n3) = chg%rho(1,  1,  1:n3)
    chg%rho_pad(0,    1:n2, 0)    = chg%rho(n1, 1:n2, n3)
    chg%rho_pad(0,    1:n2, n3+1) = chg%rho(n1, 1:n2, 1)
    chg%rho_pad(n1+1, 1:n2, 0)    = chg%rho(1,  1:n2, n3)
    chg%rho_pad(n1+1, 1:n2, n3+1) = chg%rho(1,  1:n2, 1)
    chg%rho_pad(1:n1, 0,    0)    = chg%rho(1:n1, n2, n3)
    chg%rho_pad(1:n1, 0,    n3+1) = chg%rho(1:n1, n2, 1)
    chg%rho_pad(1:n1, n2+1, 0)    = chg%rho(1:n1, 1,  n3)
    chg%rho_pad(1:n1, n2+1, n3+1) = chg%rho(1:n1, 1,  1)

    chg%rho_pad(0,    0,    0)    = chg%rho(n1, n2, n3)
    chg%rho_pad(0,    0,    n3+1) = chg%rho(n1, n2, 1)
    chg%rho_pad(0,    n2+1, 0)    = chg%rho(n1, 1,  n3)
    chg%rho_pad(0,    n2+1, n3+1) = chg%rho(n1, 1,  1)
    chg%rho_pad(n1+1, 0,    0)    = chg%rho(1,  n2, n3)
    chg%rho_pad(n1+1, 0,    n3+1) = chg%rho(1,  n2, 1)
    chg%rho_pad(n1+1, n2+1, 0)    = chg%rho(1,  1,  n3)
    chg%rho_pad(n1+1, n2+1, n3+1) = chg%rho(1,  1,  1)

  END SUBROUTINE init_rho_pad
