...
!            IF (rho_val(chgval,n1,n2,n3)/vol <= opts%vacval) THEN
    ...
    bdr%volchg = 0._q2
#ifdef _OPENACC
    !$acc data copyin(bdr%volnum, chgval%rho) copy(bdr%volchg)
    ...
    !$acc end data
#else
    !$OMP PARALLEL DO COLLAPSE(2) PRIVATE(n2, n3) SCHEDULE(static)
    DO n1 = 1, chgval%npts(1)
      DO n2 = 1, chgval%npts(2)
        DO n3 = 1, chgval%npts(3)
          IF (bdr%volnum(n1,n2,n3) == bdr%nvols+1) CYCLE
...
!-----------------------------------------------------------------------------------!
! trace_neargrid: Thread-safe gradient ascent to maximum (neargrid algorithm).
!   Uses only read-only chg. Returns the attractor grid point.
!-----------------------------------------------------------------------------------!

  SUBROUTINE trace_neargrid(chg, p_in, p_out)
    !$acc routine seq

    TYPE(charge_obj) :: chg
    INTEGER, DIMENSION(3), INTENT(IN) :: p_in
    INTEGER, DIMENSION(3), INTENT(OUT) :: p_out

    INTEGER, DIMENSION(3) :: p, pm, p_prev
    REAL(q2), DIMENSION(3) :: dr, gradrl
    REAL(q2) :: coeff
    INTEGER :: iter
    ...
    DO iter = 1, MAX_ITER
      ...
      IF (MAXVAL(ABS(gradrl)) < 1E-30_q2) THEN
        IF (is_max_ongrid(chg, p)) EXIT
        pm = p
      ...
      ELSE
        ...
        gradrl = coeff * gradrl
        pm = p + NINT(gradrl)
        dr = dr + gradrl - REAL(NINT(gradrl), q2)
        pm = pm + NINT(dr)
        dr = dr - REAL(NINT(dr), q2)
        CALL pbc(pm, chg%npts)

        IF (ALL(pm == p_prev)) THEN
          pm = p
          CALL step_ongrid(chg, pm)
          dr = (/0._q2, 0._q2, 0._q2/)
        END IF
      ...
      IF (ALL(p == p_prev)) EXIT
    END DO

    ! Final on-grid snap to ensure true maximum
    DO iter = 1, 100
      p_prev = p
  ...
  END SUBROUTINE trace_neargrid

!-----------------------------------------------------------------------------------!
! trace_ongrid: Thread-safe on-grid steepest ascent to maximum.
!-----------------------------------------------------------------------------------!

  SUBROUTINE trace_ongrid(chg, p_in, p_out)
    !$acc routine seq
    ...
    ! step_ongrid already converges to on-grid maximum
    p_out = p

  END SUBROUTINE trace_ongrid

!-----------------------------------------------------------------------------------!
! trace_offgrid: Thread-safe off-grid gradient ascent to maximum.
!-----------------------------------------------------------------------------------!

  SUBROUTINE trace_offgrid(chg, stepsize, p_in, p_out)
    !$acc routine seq

    TYPE(charge_obj) :: chg
    REAL(q2), INTENT(IN) :: stepsize
    INTEGER, DIMENSION(3), INTENT(IN) :: p_in
    INTEGER, DIMENSION(3), INTENT(OUT) :: p_out

    INTEGER, DIMENSION(3) :: p, pm
    REAL(q2), DIMENSION(3) :: r, grad, dr_car, dr_lat
    REAL(q2) :: rho, cp, cm
    INTEGER :: iter
    INTEGER, PARAMETER :: MAX_ITER = 10000
    ...
    DO iter = 1, MAX_ITER
      ...
      IF (SUM(grad*grad) < 1E-60_q2) EXIT
      dr_car = grad * stepsize / SQRT(SUM(grad*grad))
...
!-----------------------------------------------------------------------------------!
  ...
  SUBROUTINE step_neargrid(bdr,chg,p)
    ...
    IF (bdr%pnum == 1) THEN
      bdr%dr_accum = (/0._q2,0._q2,0._q2/)
    END IF
    ...
    ELSE
      ...
      gradrl = coeff*gradrl
      pm = p + ANINT(gradrl)
      bdr%dr_accum = bdr%dr_accum + gradrl - ANINT(gradrl)
      pm = pm + ANINT(bdr%dr_accum)
      bdr%dr_accum = bdr%dr_accum - ANINT(bdr%dr_accum)
    END IF
...
!   Store the minimum distance and the vector
    ...
    !$OMP PARALLEL DO COLLAPSE(2) PRIVATE(n2, n3, p, v, dv_dir, dv_car, atom, dist) &
